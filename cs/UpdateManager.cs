...
namespace AbiturEliteCode
{
    public static class UpdateManager
    {
                ...
                if (root.TryGetProperty("tag_name", out var tagElement))
                {
                    ...
                    if (Version.TryParse(CurrentVersion, out var current) && Version.TryParse(tag, out var latest))
                    {
                        if (latest > current)
                        {
                            ...
                            if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux)) targetAsset = "AbiturEliteCode-linux.zip";
                            else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX)) targetAsset = "AbiturEliteCode-mac.zip";
        ...
        public static async Task<UpdateStatus> PerformUpdateAsync(string downloadUrl, IProgress<(string message, double percentage)> progress)
        {
            ...
            {
                return UpdateStatus.UnsupportedOS;
            }

            // fallback if windows lacks write permissions
            if (!HasWritePermission(currentAppDir))
            {
                return UpdateStatus.NoWritePermission;
            }
            ...
            try
            {
                ...
                foreach (var file in tempSaveFiles)
                {
                    File.Delete(file);
                }
                ...
                string currentExe = Process.GetCurrentProcess().MainModule?.FileName ?? Path.Combine(currentAppDir, "AbiturEliteCode.exe");
...
";
        ...
        private static async Task DownloadFileAsync(string url, string destination, IProgress<(string message, double percentage)> progress)
        {
            using var client = new HttpClient();
            client.DefaultRequestHeaders.UserAgent.ParseAdd("AbiturEliteCode-Updater");
